...
def printAppScanverison(APPSCAN_VER, BANNER):
    if platform.system()=="Windows":
        print '\n\nMobile Security Framework '+ APPSCAN_VER
    else:
        print '\n\n\033[1m\033[34mMobile Security Framework '+ APPSCAN_VER +'\033[0m'
    print BANNER

def createUserConfig(AppScan_HOME):
    try:
        CONFIG_PATH = os.path.join(AppScan_HOME,'config.py')
        if isFileExists(CONFIG_PATH) == False:
            SAMPLE_CONF = os.path.join(settings.BASE_DIR,"AppScan/settings.py")
...
def getAppScanHome(useHOME):
    try:
        AppScan_HOME = ""
        if useHOME:
            AppScan_HOME = os.path.join(os.path.expanduser('~'),".AppScan")
            #AppScan Home Directory
            if not os.path.exists(AppScan_HOME):
                os.makedirs(AppScan_HOME)
            createUserConfig(AppScan_HOME)
        else:
            AppScan_HOME = settings.BASE_DIR
        #Logs Directory
        ...
        PrintException("[ERROR] Creating AppScan Home Directory")
...
def PrintException(msg,web=False):
    ...
    with open(LOGPATH + 'AppScan.log','a') as f:
        f.write(dat)
